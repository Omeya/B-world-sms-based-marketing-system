...
public class Signup extends Activity implements IResultCodeConstants,OnClickListener,URLConstants,IResponseConstants,IJSONTagConstants,IImageConstants
{
	...
	class SignupProcess extends AsyncTask<Void, Void, Void>
	{
		...
		APIManager manager;
		JSONObject resObj;
		private String [] name={"username","password","name","phonenumber","dob","gender"};
		private String [] values={edtEmail.getText().toString(),edtPassword.getText().toString()
				,edtFirstname.getText().toString(),edtPhone.getText().toString(),
				txtBirthday.getText().toString(),mGender.getSelectedItem().toString()};
		...
		public void onDateSet(DatePicker view, int selectedYear,int selectedMonth, int selectedDay)
		{
			...
			switch(viewID)
			{
				case R.id.txtBirthday:
					...
					txtBirthday.setText(GlobalConstants.year+"-"+GlobalConstants.month+"-"+GlobalConstants.day);
					break;
	...
	public void onClick(View v)
	{
		if(v==btnJoin)
		{
			...
			else if(edtPhone.getText().toString().length()<1)
			{
				AlertManager.shorttoastMessage(Signup.this, getString(R.string.Enter_phone));
			}
